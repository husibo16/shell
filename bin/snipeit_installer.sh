...
# 用途: 在 Ubuntu Server 上完整部署 Snipe-IT（含 Node.js、Git、Redis、Supervisor、SMTP 邮箱配置）
# 版本: 3.2.2
# 作者: 胡博涵
# 更新: 2025-08-28
# 许可: MIT
...
MAIL_USERNAME="${MAIL_USERNAME:-YOURUSERNAME}"
...
MAIL_REPLYTO_ADDR="${MAIL_REPLYTO_ADDR:-noreply@example.com}"
...
# 注意：Snipe-IT 现用 Symfony Mailer，已不再使用 MAIL_ENCRYPTION
...
SNIPEIT_VERSION="${SNIPEIT_VERSION:-v8.2.1}"
...
banner() {
  local text="$1"
  local colors=("${_c_r}" "${_c_y}" "${_c_g}" "${_c_c}" "${_c_b}" "${_c_m}")
  local out="" c
  for ((i = 0; i < ${#text}; i++)); do
    c=${colors[$((i % ${#colors[@]}))]}
    out+="${c}${text:i:1}${_c_reset}"
  done
  echo -e "$out"
}
...
子命令（中文交互友好）:
  ...
  一键全装               按最佳顺序执行全部步骤
  查看日志_logs          查看 Laravel 最近 50 行日志
...
interactive_menu() {
  ...
  echo -e "${_c_g}请选择操作（输入序号）:${_c_reset}"
  PS3="👉 你的选择: "
  local opts=(
    "体检_precheck"
    "安装_db_mariadb"
...
cmd_install_php() {
  ...
  apt install -y "php${pv}-fpm" "php${pv}-cli" "php${pv}-common" \
    ...
    "php${pv}-mysql" "php${pv}-xml" "php${pv}-zip" "php${pv}-intl" "php${pv}-readline" "php${pv}-redis"
  phpenmod opcache redis || true
...
cmd_snipe_download() {
  ...
  if [[ ! -d "$APP_DIR/.git" ]]; then
    git clone --depth 1 --branch "$SNIPEIT_VERSION" https://github.com/snipe/snipe-it.git "$APP_DIR"
  else
    git -C "$APP_DIR" fetch --tags
    git -C "$APP_DIR" checkout "$SNIPEIT_VERSION"
    git -C "$APP_DIR" pull --ff-only origin "$SNIPEIT_VERSION" || true
  fi
  cd "$APP_DIR"
...
cmd_snipe_config_env() {
  ...
  echo -e "${_c_y}邮件（SMTP）设置：${_c_reset}"
...
cmd_snipe_dependencies() {
  ...
  mkdir -p /var/www/.cache/composer /var/www/.npm
  chown -R www-data:www-data /var/www/.cache /var/www/.npm
  sudo -H -u www-data composer install --no-dev --prefer-dist --optimize-autoloader
  if command -v npm >/dev/null 2>&1; then
    sudo -H -u www-data npm ci --no-audit --no-fund
    sudo -H -u www-data npm run build || sudo -H -u www-data npm run prod || log_warn "npm 构建失败"
  else
    log_warn "未检测到 Node.js，跳过前端构建"
  fi
  sudo -H -u www-data php artisan key:generate || true
  if confirm "导入演示数据（包含示例资产等）?"; then
    sudo -H -u www-data php artisan migrate --seed --force
  else
    sudo -H -u www-data php artisan migrate --force
  fi
  chown -R www-data:www-data "$APP_DIR"
...
cmd_test_smtp() {
  ...
  echo -e "${_c_b}提示：登录 Snipe-IT 后台→设置→邮件，发“测试邮件”验证账号/证书策略。${_c_reset}"
}
...
cmd_install_all() {
  ...
  echo -e "${_c_g}首次进入初始化向导，创建管理员账户；随后建议开启 HTTPS 与定时备份。${_c_reset}"
}
